...
import { Menu, X, ChevronDown, Smartphone, Laptop, ChevronRight } from 'lucide-react';

export default function Navbar() {
    const [isScrolled, setIsScrolled] = useState(false);
    const [mobileMenuOpen, setMobileMenuOpen] = useState(false);

    // Handle scroll for transparent -> solid background
    useEffect(() => {
    ...
    return (
        <>
            {/* 
                Increased z-index to z-[100] to ensure no hero sections overlap it. 
                This solves the issue of the hamburger not being clickable at the top.
            */}
            <nav className={`fixed w-full z-[100] transition-all duration-300 ${isScrolled ? 'bg-white shadow-sm py-3 text-[#0F172A]' : 'bg-transparent py-5 text-white'}`}>
                    ...
                    <div className="flex justify-between items-center relative">
                        ...
                        <div className="md:hidden flex items-center">
                            <button
                                ...
                                className={`p-2 rounded-lg focus:outline-none transition-colors ${isScrolled ? 'text-[#0F172A] hover:bg-gray-100' : 'text-white hover:bg-white/10'}`}
            ...
            {/* Mobile Menu Overlay */}
            <div className={`fixed inset-0 bg-white z-[110] transform transition-transform duration-300 ease-in-out flex flex-col ${mobileMenuOpen ? 'translate-y-0' : '-translate-y-full'}`}>

                {/* Mobile Header */}
                <div className="flex justify-between items-center p-5 border-b border-gray-100">
                    <Link to="/" onClick={() => setMobileMenuOpen(false)} className="font-display font-bold text-2xl tracking-tight text-[#0F172A]">
                        SolvIQ
                    </Link>
                    <button
                        onClick={() => setMobileMenuOpen(false)}
                        className="p-2 rounded-lg text-[#0F172A] hover:bg-gray-100 transition-colors focus:outline-none"
                ...
                <div className="flex-1 overflow-y-auto px-5 py-6 pb-24 space-y-8">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <Link to="/prepone" onClick={() => setMobileMenuOpen(false)} className="bg-[#EEF2FF] text-[#4F46E5] p-4 rounded-2xl flex flex-col items-center justify-center text-center space-y-2 active:bg-[#e0e7ff] transition-colors">
                            <Smartphone className="w-7 h-7" />
                            <span className="font-display font-bold text-sm">For Students</span>
                        </Link>
                        <Link to="/coachone" onClick={() => setMobileMenuOpen(false)} className="bg-[#f7f6f2] text-[#28251d] p-4 rounded-2xl flex flex-col items-center justify-center text-center space-y-2 active:bg-[#e8e6df] transition-colors">
                            <Laptop className="w-7 h-7" />
                            <span className="font-display font-bold text-sm">For Institutes</span>
                        </Link>
                    </div>

                    {/* Students Links */}
                    <div>
                        <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-4 px-2">For Students</h3>
                        <div className="bg-gray-50 rounded-2xl overflow-hidden border border-gray-100">
                            {studentLinks.map((link, idx) => (
                                <Link
                                    ...
                                    to={link.path}
                                    onClick={() => setMobileMenuOpen(false)}
                                    className={`flex items-center justify-between px-4 py-4 text-[#0F172A] active:bg-gray-200 transition-colors ${idx !== studentLinks.length - 1 ? 'border-b border-gray-200/60' : ''}`}
                                >
                    ...
                    {/* Institutes Links */}
                    <div>
                        <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-4 px-2">For Institutes</h3>
                        <div className="bg-gray-50 rounded-2xl overflow-hidden border border-gray-100">
                            {instituteLinks.map((link, idx) => (
                                <Link
                                    ...
                                    to={link.path}
                                    onClick={() => setMobileMenuOpen(false)}
                                    className={`flex items-center justify-between px-4 py-4 text-[#0F172A] active:bg-gray-200 transition-colors ${idx !== instituteLinks.length - 1 ? 'border-b border-gray-200/60' : ''}`}
                                >
                                    <span className="font-medium text-sm">{link.name}</span>
                                    <ChevronRight className="w-4 h-4 text-gray-400" />
                                </Link>
                    ...
                    {/* Company Links */}
                    <div>
                        <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-4 px-2">Company</h3>
                        <div className="bg-gray-50 rounded-2xl overflow-hidden border border-gray-100 flex flex-col">
                            <Link to="/blog" onClick={() => setMobileMenuOpen(false)} className="flex items-center justify-between px-4 py-4 text-[#0F172A] active:bg-gray-200 transition-colors border-b border-gray-200/60">
                                <span className="font-medium text-sm">Blog</span>
                                <ChevronRight className="w-4 h-4 text-gray-400" />
                            </Link>
                            <Link to="/about" onClick={() => setMobileMenuOpen(false)} className="flex items-center justify-between px-4 py-4 text-[#0F172A] active:bg-gray-200 transition-colors">
